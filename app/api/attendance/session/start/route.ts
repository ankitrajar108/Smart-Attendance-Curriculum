...
export async function POST(req: NextRequest) {
  try {
    ...
    const { timetableSlotId, durationMinutes = 5 } = await req.json();
    if (!timetableSlotId) {
      return NextResponse.json(
        { message: "timetableSlotId is required" },
        { status: 400 },
      );
    }

    const clampedMinutes = Math.min(
      Math.max(Number(durationMinutes) || 5, 1),
      60,
    );
    ...
    const expiresAt = Date.now() + clampedMinutes * 60 * 1000;
